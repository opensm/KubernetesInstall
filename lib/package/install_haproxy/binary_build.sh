function install() {
  ...
  if [ ! -f 'haproxy.service' ]; then
    echo "启动脚本不存在：$download_path/haproxy.service"
    exit 1
  fi
  # shellcheck disable=SC2164
  tar -zxvf "${package}".tar.gz && cd haproxy && make PREFIX="$tmp_install" TARGET=linux2628 && make install PREFIX="$tmp_install"
